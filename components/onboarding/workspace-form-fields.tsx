  ...
  return (
    <fieldset disabled={disabled} className="space-y-10 disabled:opacity-70">
      <Group title="O vás a vašom podnikaní">
        ...
        <div className="sm:col-span-2"><Field multiline label="Povedzte mi trochu viac o vašom podnikaní a o tom, čomu sa venujete." value={answers.business.description} onChange={(description) => onChange({ ...answers, business: { ...answers.business, description } })} /></div>
      ...
      <Group title="Zákazníci a cieľ webu">
        ...
        <div className="sm:col-span-2"><Field multiline label="Čo viete zo skúseností o svojich zákazníkoch – čo najviac riešia, oceňujú alebo sa pýtajú?" value={answers.customerInsights} onChange={(customerInsights) => onChange({ ...answers, customerInsights })} /></div>
        <div className="sm:col-span-2"><Field multiline label="Aké najčastejšie obavy má zákazník pred objednávkou?" hint="Napr. cena, termín, kvalita výsledku, dôvera, reklamácia alebo neistota z výsledku." value={answers.customerConcerns} onChange={(customerConcerns) => onChange({ ...answers, customerConcerns })} /></div>
        <div className="sm:col-span-2"><Field multiline label="Akú konkrétnu reakciu zákazníka by ste chceli po návšteve webu?" hint="Napr. „Títo presne vedia, čo robia.“" value={answers.desiredCustomerReaction} onChange={(desiredCustomerReaction) => onChange({ ...answers, desiredCustomerReaction })} /></div>
        <ChoiceField title={`Čo chcete pomocou nového webu dosiahnuť?${hint('websiteExpectations') ? ' · Predvyplnené z predchádzajúcej komunikácie' : ''}`} options={websiteExpectationOptions} selected={answers.websiteExpectations} onChange={(websiteExpectations) => emit({ ...answers, websiteExpectations }, 'websiteExpectations')}><OtherAnswer show={answers.websiteExpectations.includes('Iné')} label="Iný cieľ" value={answers.websiteExpectationsOther} onChange={(websiteExpectationsOther) => onChange({ ...answers, websiteExpectationsOther })} /></ChoiceField>
        <div className="sm:col-span-2"><Field multiline label="Prečo je pre vás tento cieľ dôležitý?" value={answers.goalImportance} onChange={(goalImportance) => onChange({ ...answers, goalImportance })} /></div>
        <div className="sm:col-span-2"><Field multiline label="Podľa čoho spoznáte, že nový web funguje a ste s ním spokojný?" value={answers.successCriteria} onChange={(successCriteria) => onChange({ ...answers, successCriteria })} /></div>
        <div className="sm:col-span-2"><Field multiline label="Čo je pre vás na novom webe najdôležitejšie?" value={answers.websitePriorities} onChange={(websitePriorities) => onChange({ ...answers, websitePriorities })} /></div>
      ...
      <Group title="Vizuálny smer">
        <div className="sm:col-span-2"><Field multiline label="Čo chcete, aby si človek o vašej firme pomyslel po 5 sekundách na webe?" hint="Prvá intuitívna reakcia – ešte predtým, než začne čítať detaily." value={answers.brandFirstImpression} onChange={(brandFirstImpression) => onChange({ ...answers, brandFirstImpression })} /></div>
        <ChoiceField title="Aké 3–5 slov má vaša značka reprezentovať?" options={includeSavedOptions(brandAttributeOptions, answers.designPreferences)} selected={answers.designPreferences} onChange={(designPreferences) => onChange({ ...answers, designPreferences })}><OtherAnswer show={answers.designPreferences.includes('Iné')} label="Iné slovo" value={answers.designOther} onChange={(designOther) => onChange({ ...answers, designOther })} /></ChoiceField>
        ...
        <ChoiceField title="Čím vaša značka určite nemá byť?" options={dislikeOptions} selected={answers.designDislikes} onChange={(designDislikes) => onChange({ ...answers, designDislikes })}><OtherAnswer show={answers.designDislikes.includes('Iné')} multiline label="Iná neželaná vlastnosť" value={answers.dislikes} onChange={(dislikes) => onChange({ ...answers, dislikes })} /></ChoiceField>
        <div className="sm:col-span-2"><Field multiline label="Je niečo, čo musím pri návrhu rešpektovať alebo o čom by som mal vedieť?" value={answers.projectConstraints} onChange={(projectConstraints) => onChange({ ...answers, projectConstraints })} /></div>
      ...
      <Group title="Spolupráca">
        <div className="sm:col-span-2"><Field multiline label="Ako veľmi chcete byť zapojený do návrhu a jednotlivých rozhodnutí?" value={answers.collaborationInvolvement} onChange={(collaborationInvolvement) => onChange({ ...answers, collaborationInvolvement })} /></div>
        <div className="sm:col-span-2"><Field multiline label="Ako vám najviac vyhovuje komunikovať a dávať spätnú väzbu?" value={answers.feedbackCommunication} onChange={(feedbackCommunication) => onChange({ ...answers, feedbackCommunication })} /></div>
      </Group>
      <Group title="Kontakt a fakturácia">
        ...
        <div className="sm:col-span-2"><Field hint={hint('additionalNotes')} multiline label="Je ešte niečo dôležité, na čo som sa nespýtal a mal by som to pred návrhom webu vedieť?" value={answers.additionalNotes} onChange={(additionalNotes) => emit({ ...answers, additionalNotes }, 'additionalNotes')} /></div>
      </Group>
    </fieldset>
  )
}
